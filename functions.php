...
function wpna_styles() {
	...
	wp_enqueue_style( 'wpna-gfonts', 'https://fonts.googleapis.com/css?family=Lato:300,400,700,300italic,400italic', null, '1.0.0', 'all' );
}
add_action( 'wp_enqueue_scripts', 'wpna_styles' );

function wpna_scripts() {
	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'wpna-tweenmax', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/latest/TweenMax.min.js', array(), '1.0.0', true );
	wp_enqueue_script( 'wpna-ScrollToPlugin', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/latest/plugins/ScrollToPlugin.min.js', array(), '1.0.0', true );
